<?php 
	...
	class Game {
		...
		public function display() {
			...
			echo '<h2>'.$res2->fetch()['name'].' de '.$res->fetch()['name'].'</h2>';
			echo '<form action="pronostiquer.php" method="post">';
			
			$requete = "SELECT id FROM pronos where user = {$this->id_user} and game = {$this->id}";
			$checkProno = $this->database->query($requete);
			$fCheckProno = $checkProno->fetch();
			
			if (empty($fCheckProno)) {
				echo '<p class="p-wait">En attente</p>';
			}
			else {
				echo '<p class="p-ok">Validé</p>';
			}
			...
			if (!empty($this->player1) && $id_joueur != -1) {
				...
				$requete = "SELECT name, flag FROM players WHERE id = ".$id_joueur;
				$res = $this->database->query($requete);
				$fRes = $res->fetch();
